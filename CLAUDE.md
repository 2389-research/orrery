...
- **SIGILL (exit 132) on Apple Silicon**: on a Mac under Docker Desktop, importing `cryptography` / `pypdf` / `simmer-sdk` can crash a container with SIGILL — the VM advertises ARM crypto features (SME/SVE2) it can't execute, so OpenSSL faults. Fix is `OPENSSL_armcap=0` (set in `docker-compose.ollama.yml` for the Mac-local stack; NOT on real Linux like the Spark, which keeps hardware crypto). Full writeup + when-to-use table: `docs/troubleshooting-local-dev.md`.
